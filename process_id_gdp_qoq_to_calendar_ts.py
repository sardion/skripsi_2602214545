...
# -*- coding: utf-8 -*-
"""
process_id_gdp_qoq_to_calendar_ts.py

Build a daily calendar time series for Indonesia Core GDP QoQ with:
- calendar_date
- id_gdp_qoq (string, preserves % as in raw CSV)
- days_since_id_gdp_qoq (int)

Input:
- data/raw/investing_id_gdp_qoq_2020-2025_raw.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/id_gdp_qoq_ts_cal.csv
...
INPUT_FILENAME: Final[str] = "investing_id_gdp_qoq_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "id_gdp_qoq_ts_cal.csv"

INPUT_CSV: Final[Path] = RAW_DIR / INPUT_FILENAME
OUTPUT_CSV: Final[Path] = PROCESSED_DIR / OUTPUT_FILENAME

CAL_START: Final[date] = date(2020, 1, 1)
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build id_gdp_qoq as strings (preserve '%').
...
def main() -> None:
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "id_gdp_qoq": series_vals,                      # keep % strings
        "days_since_id_gdp_qoq": days_since_series,     # int
    })
