...
# -*- coding: utf-8 -*-
"""
process_id_inflation_mom_to_calendar_ts.py

Build a daily calendar time series for Indonesia Inflation MoM with:
- calendar_date
- id_inflation_mom (string, preserves % as in raw CSV)
- days_since_id_inflation_mom (int)

Input:
- data/raw/investing_id_inflation_mom_2020-2025_raw.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/id_inflation_mom_ts_cal.csv
...
INPUT_FILENAME: Final[str] = "investing_id_inflation_mom_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "id_inflation_mom_ts_cal.csv"
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build id_inflation_mom as strings (preserve '%').
...
def main() -> None:
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "id_inflation_mom": series_vals,                      # keep % strings
        "days_since_id_inflation_mom": days_since_series,     # int
    })
