...
# -*- coding: utf-8 -*-
"""
process_us_core_cpi_mom_to_calendar_ts.py

Build a daily calendar time series for US Core CPI MoM with:
- calendar_date
- us_core_cpi_mom (string, preserves % as in raw CSV)
- days_since_us_core_cpi_mom (int)

Input:
- data/raw/investing_us_core_cpi_mom_raw_2020-2025.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/us_core_cpi_mom_ts_cal.csv
...
INPUT_FILENAME: Final[str] = "investing_us_core_cpi_mom_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "us_core_cpi_mom_ts_cal.csv"
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build us_core_cpi_mom as strings (preserve '%').
...
def main() -> None:
    ...
    cpi_series: List[str] = build_value_series(calendar_days, releases)
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "us_core_cpi_mom": cpi_series,                      # keep % strings
        "days_since_us_core_cpi_mom": days_since_series,    # int
    })
