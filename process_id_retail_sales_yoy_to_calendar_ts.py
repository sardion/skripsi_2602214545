...
# -*- coding: utf-8 -*-
"""
process_id_retail_sales_yoy_to_calendar_ts.py

Build a daily calendar time series for Indonesia Retail Sales YoY with:
- calendar_date
- id_retail_sales_yoy (string, preserves % as in raw CSV)
- days_since_id_retail_sales_yoy (int)

Input:
- data/raw/investing_id_retail_sales_yoy_2020-2025_raw.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/id_retail_sales_yoy_ts_cal.csv
...
- days_since_id_retail_sales_yoy:
...
INPUT_FILENAME: Final[str] = "investing_id_retail_sales_yoy_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "id_retail_sales_yoy_ts_cal.csv"
...
def _parse_dates_multi_format(date_series: pd.Series) -> pd.Series:
    """
    Deterministic multi-format parser to avoid warnings and ambiguity.
    Tries %m/%d/%Y → %d/%m/%Y → %Y-%m-%d; falls back to general parser (raise on failure).
    Returns pandas Timestamps normalized to midnight.
    """
    # 1) MDY (e.g., 7/31/2025)
    parsed = pd.to_datetime(date_series, format="%m/%d/%Y", errors="coerce")
    na_mask = parsed.isna()

    # 2) DMY (e.g., 31/07/2025)
    if na_mask.any():
        parsed_dmy = pd.to_datetime(date_series[na_mask], format="%d/%m/%Y", errors="coerce")
        parsed.loc[na_mask & parsed_dmy.notna()] = parsed_dmy
        na_mask = parsed.isna()

    # 3) ISO (YYYY-MM-DD)
    if na_mask.any():
        parsed_iso = pd.to_datetime(date_series[na_mask], format="%Y-%m-%d", errors="coerce")
        parsed.loc[na_mask & parsed_iso.notna()] = parsed_iso
        na_mask = parsed.isna()

    # 4) Last resort: general parser (raises on failure to keep determinism)
    if na_mask.any():
        parsed_general = pd.to_datetime(date_series[na_mask], errors="raise")
        parsed.loc[na_mask] = parsed_general
...
def read_release_rows(csv_path: Path) -> List[ReleaseRow]:
    """
    Read raw CSV, parse dates deterministically across common formats,
    ...
    # Deterministic, multi-format date parsing without warnings
    parsed = _parse_dates_multi_format(df["Date"])
    df["__date"] = parsed.dt.date
    ...
    for _, row in dedup.iterrows():
        d: date = row["__date"]
        t: dtime = row["__time"]
        a: str = str(row.get("Actual", "")).strip()
        p: str = str(row.get("Previous", "")).strip()
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build id_retail_sales_yoy as strings (preserve '%').
...
def main() -> None:
    ...
    series_vals: List[str] = build_value_series(calendar_days, releases)
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "id_retail_sales_yoy": series_vals,                    # keep % strings
        "days_since_id_retail_sales_yoy": days_since_series,   # int
    })
