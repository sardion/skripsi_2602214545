...
def _unwrap_dataset(ds: Any) -> Any:
    ...
    ----------
    ds : Any
        Possibly wrapped dataset.

    Returns
    -------
    Any
        Unwrapped, base dataset object.
    """
...
def _get_denorm_fn(dataset: Any) -> Optional[Callable[[np.ndarray], np.ndarray]]:
    ...
    ----------
    dataset : Any
        Base dataset (unwrapped).

    Returns
    -------
    Optional[Callable[[np.ndarray], np.ndarray]]
        Function that maps normalized targets back to IDR scale, or None if
        no denormalization metadata is available.
    """
...
def _stable_mape(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    The denominator is max(|y_true|, floor), where floor is 1% of the median
    absolute(y_true) or 1e-8, whichever is larger.

    Parameters
    ----------
    y_true : np.ndarray
        Ground truth values (preferably in original IDR scale).
    y_pred : np.ndarray
        Predictions (same scale as y_true).

    Returns
    -------
    float
        MAPE in percent. Returns NaN if y_true is empty.
    """
    if y_true.size == 0:
        return float("nan")
...
def evaluate(
    ...
    *,
    ...
    with torch.no_grad():
        for X, y in dl:
            # Move batch to device (supports dict or tensor)
...
def save_metrics(
    ...
    ----------
    results_dir : Path
        Base results directory for the scenario (contains 'metrics' subdir).
    split_name : str
        Split identifier, e.g., 'close_val', 'close_test', 'mean_val'.
    result : Any
        EvalResult or any object exposing attributes rmse/mae/mape as floats.
    ticker : str
        Stock code (e.g., 'BBCA').
    verbose : bool, optional
        If True, print output paths. Default is False.
    """
