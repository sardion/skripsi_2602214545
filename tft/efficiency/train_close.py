...
# -*- coding: utf-8 -*-
"""
Efficiency scenario trainer (no early stopping, fixed epochs) for TFT CLOSE target.
...
def main() -> None:
    ...
    start_msg = (f"[train_close][TFT efficiency] Running on device: {device}")
    print(start_msg)
    ...
    if device.type == "cuda":
        ...
        device_msg = ("[train_close][TFT efficiency] CUDA available, running on GPU (AMP enabled, torch profiling).")
    else:
        device_msg = ("[train_close][TFT efficiency] CUDA not available, running on CPU (AMP disabled, CPU-only profiling).")
    print(device_msg)
    ...
    # Time
    (results_dir / "logs" / f"{ticker}_efficiency_close_total_elapsed_s.txt").write_text(
        f"{total_elapsed:.2f}\n", encoding="utf-8"
    )
    ...
    if device.type == "cuda":
        ...
        (results_dir / "logs" / f"{ticker}_efficiency_close_peak_cuda_memory_mb.txt").write_text(
            f"{peak_mem_mb:.2f}\n", encoding="utf-8"
        )
    else:
        mem_info = proc.memory_info()
        peak_mb = getattr(mem_info, "peak_wset", mem_info.rss) / (1024 ** 2)
        (results_dir / "logs" / f"{ticker}_efficiency_close_peak_cpu_memory_mb.txt").write_text(
            f"{peak_mb:.2f}\n", encoding="utf-8"
        )
    ...
    (results_dir / "logs" / f"{ticker}_efficiency_close_cpu_utilization_pct.txt").write_text(
        f"{cpu_util_pct:.2f}\n", encoding="utf-8"
    )
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_close][TFT efficiency] Done on device: {device}\n")
