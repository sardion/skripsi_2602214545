...
# -*- coding: utf-8 -*-
"""
TFT stopping_no_embargo: test script for MEAN target (IDR-scale metrics).

Conventions aligned with train_mean.py (TFTLite-style):
- Deterministic make_dataloaders(...)
- y_min/y_max attach for MEAN target (from JSON or RAW CSV; AMP fallback)
- Safe checkpoint load (weights_only=True with fallback)
- Single [SPLIT] line (silences internal prints)
- Quiet save_metrics; single artifact summary trailer
...
def attach_y_minmax_from_metadata_or_raw_mean(ds: Dataset[Any], ticker: str, project_root: Path) -> None:
    ...
    Priority:
      ...
      2) RAW CSV: compute min/max from 'mean_price' or AMP fallback from OHLC
    """
    meta_candidates = [
...
def main() -> None:
    ...
    start_msg = f"[TFT stopping_no_embargo][test_mean] Running on device: {device}"
    print(start_msg)
    ...
    # Dataset (TFT-specific loader)
    ds = load_dataset_tft(ticker, target_kind, cfg)

    # Attach y_min/y_max for IDR-scale metrics (mean)
