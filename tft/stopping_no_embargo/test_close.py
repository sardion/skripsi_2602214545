...
# -*- coding: utf-8 -*-
"""
TFT stopping_no_embargo: test script for CLOSE target (IDR-scale metrics).

Conventions aligned with train_close.py (TFTLite):
...
def make_dataloaders(
    ...
    deterministic train shuffle like train_close.py.
    """
...
class _EvalAdapter(torch.nn.Module):
    """
    Adapter so metrics.evaluate(model, loader, ...) — which calls model(X) —
    works with TFT dict-batches without changing evaluate().
    """
...
def main() -> None:
    ...
    start_msg = f"[TFT stopping_no_embargo][test_close] Running on device: {device}"
    print(start_msg)
    ...
    # Model + adapter (to accept dict batches in evaluate)
    core = TFTLite(
        d_past=d_past,
        d_fut=d_fut,
        d_static=d_static,
    ...
    # Load best checkpoint (same name pattern as training)
    ...
    # Evaluate on VAL (IDR scale via explicit y_min/y_max)
