...
# -*- coding: utf-8 -*-
"""
TFT training script (stopping_no_embargo): mean-price (AMP) target.
...
from model import TFTLite  # revised TFT model (past/future/static)
from metrics import EvalResult, save_metrics, evaluate
from figures import plot_pred_vs_true, plot_loss_curve

# ---- silence the harmless PyTorch LSTM dropout warning for num_layers=1 ----
...
def attach_y_minmax_from_metadata_or_raw_mean(ds: Dataset[Any], ticker: str, project_root: Path) -> None:
    ...
    Priority:
      ...
      2) RAW CSV: compute min/max from 'mean_price' or OHLC → AMP
    """
    meta_candidates = [
...
def main() -> None:
    ...
    start_msg = (f"[train_mean][TFT stopping_no_embargo] Running on device: {device}")
    print(start_msg)
    ...
    # Dataset (TFT-specific loader, returns dict batches)
    target_kind: str = "mean"
    ds = load_dataset_tft(args.ticker, target_kind, cfg)

    # Attach y_min/y_max for IDR-scale metrics (mean_price)
    project_root = Path(__file__).resolve().parents[2]
    attach_y_minmax_from_metadata_or_raw_mean(ds, args.ticker, project_root)
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        # Validation (denorm via y_min/y_max attached to dataset)
        _, _, m_val = evaluate(
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_mean] [TFT stopping_no_embargo] Done on device: {device}\n")
