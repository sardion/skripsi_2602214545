...
# -*- coding: utf-8 -*-
"""
TCN training script (stopping_no_embargo): close-price target.

Conventions aligned with LSTM:
...
def attach_y_minmax_from_metadata_or_raw(ds: Dataset[Any], ticker: str, project_root: Path) -> None:
    """
    Attach y_min/y_max (IDR) for CLOSE PRICE so evaluation can denormalize.
...
def evaluate_tcn(
    ...
    *,
    ...
    """
    Evaluation that matches metrics.evaluate() behavior BUT
    performs (B,T,F)->(B,F,T) before the forward pass (TCN requirement),
    and denormalizes to IDR if y_min/y_max provided (like LSTM path).
    ...
    if y_true.size:
        ...
        # stabilized MAPE (same logic as metrics._stable_mape)
...
def main() -> None:
    ...
    start_msg = (f"[train_close][TCN stopping_no_embargo] Running on device: {device}")
    print(start_msg)
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        save_metrics(results_dir, "close_val",
                     result=type("EvalResultLike", (), m_val)(),  # tiny shim to match save_metrics dataclass shape
                     ticker=args.ticker,
                     verbose=False)
    ...
    save_metrics(results_dir, "close_test",
    ...
    # One-time artifact summary (match LSTM trailer)
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_close] [TCN stopping_no_embargo] Done on device: {device}\n")
