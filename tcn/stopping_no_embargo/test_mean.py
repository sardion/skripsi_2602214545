...
# -*- coding: utf-8 -*-
"""
TCN stopping_no_embargo: test script for MEAN target (IDR-scale metrics).

This version matches the LSTM test_* conventions:
...
def attach_y_minmax_from_metadata_or_raw_mean(
    ds: Dataset[Any],
    ticker: str,
    project_root: Path,
) -> None:
    ...
    Priority:
      ...
      2) RAW CSV: use 'mean_price' if present; otherwise fallback to (O+H+L+C)/4
    """
    meta_candidates = [
        ...
        if p.exists():
            try:
                ...
                if "mean_price" in df.columns:
                    ...
                    # Fallback to classic AMP definition
                    s = (
...
def main() -> None:
    ...
    start_msg = f"[TCN stopping_no_embargo][test_mean] Running on device: {device}"
    print(start_msg)
