...
# -*- coding: utf-8 -*-
"""
Efficiency scenario trainer (no early stopping, fixed epochs) for TCN MEAN target.
...
def attach_y_minmax_from_metadata_or_raw(ds: Any, ticker: str, project_root: Path) -> None:
    ...
    Priority:
      1) normalization metadata JSON: try mean/AMP keys
      2) RAW CSV: use mean/AMP column if available, else compute (O+H+L+C)/4
    """
    meta_candidates = [
    ...
    mean_keys = ("mean_price", "amp_price")
                ...
                for key in mean_keys:
                    if key in df.columns:
                        ...
                        if y_max > y_min:
                            ...
                            return
                # Fallback: compute mean_price from OHLC
...
def main() -> None:
    ...
    start_msg = (f"[train_mean][TCN efficiency] Running on device: {device}")
    print(start_msg)
    ...
    if device.type == "cuda":
        ...
        device_msg = ("[train_mean][TCN efficiency] CUDA available, running on GPU (AMP enabled, torch profiling).")
    else:
        device_msg = ("[train_mean][TCN efficiency] CUDA not available, running on CPU (AMP disabled, CPU-only profiling).")
    print(device_msg)
    ...
    # Attach y_min/y_max for denorm-aware metrics (MEAN)
    ...
    # Time
    (results_dir / "logs" / f"{ticker}_efficiency_mean_total_elapsed_s.txt").write_text(f"{total_elapsed:.2f}\n", encoding="utf-8")

    # Peak memory
    if device.type == "cuda":
        peak_mem_mb = torch.cuda.max_memory_allocated() / (1024 ** 2)  # type: ignore[attr-defined]
        (results_dir / "logs" / f"{ticker}_efficiency_mean_peak_cuda_memory_mb.txt").write_text(f"{peak_mem_mb:.2f}\n", encoding="utf-8")
    else:
        mem_info = proc.memory_info()
        peak_mb = getattr(mem_info, "peak_wset", mem_info.rss) / (1024 ** 2)
        (results_dir / "logs" / f"{ticker}_efficiency_mean_peak_cpu_memory_mb.txt").write_text(f"{peak_mb:.2f}\n", encoding="utf-8")
    ...
    (results_dir / "logs" / f"{ticker}_efficiency_mean_cpu_utilization_pct.txt").write_text(f"{cpu_util_pct:.2f}\n", encoding="utf-8")
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_mean][TCN efficiency] Done on device: {device}\n")
