...
# -*- coding: utf-8 -*-
"""
Efficiency scenario trainer (no early stopping, fixed epochs) for TCN CLOSE target.

Artifacts (all ticker-prefixed, under results/):
- logs/{TICKER}_train_close_efficiency_terminal_log.txt
- logs/{TICKER}_trace_{cpu|cuda}.json
- logs/{TICKER}_profiler_{cpu|cuda}.txt
- logs/{TICKER}_efficiency_close_total_elapsed_s.txt
- logs/{TICKER}_efficiency_close_peak_{cpu|cuda}_memory_mb.txt
- logs/{TICKER}_efficiency_close_cpu_utilization_pct.txt
- figures: pred_vs_true (close_val, close_test) + loss curves
- metrics: close_val.(json|txt), close_test.(json|txt)
"""
...
from metrics import evaluate, save_metrics            # denorm-aware shared evaluate
...
class ChannelsFirstWrapper(nn.Module):
    ...
    Lets us reuse the shared metrics.evaluate (which assumes (B,T,F) input).
    """
...
def attach_y_minmax_from_metadata_or_raw(ds: Any, ticker: str, project_root: Path) -> None:
    """
...
def main() -> None:
    ...
    start_msg = (f"[train_close][TCN efficiency] Running on device: {device}")
    print(start_msg)

    results_dir = Path("results")
    for sub in ("metrics", "figures", "logs", "checkpoints"):
        (results_dir / sub).mkdir(parents=True, exist_ok=True)
    log_path = results_dir / "logs" / f"{ticker}_train_close_efficiency_terminal_log.txt"
    ...
    if device.type == "cuda":
        ...
        device_msg = ("[train_close][TCN efficiency] CUDA available, running on GPU (AMP enabled, torch profiling).")
    else:
        device_msg = ("[train_close][TCN efficiency] CUDA not available, running on CPU (AMP disabled, CPU-only profiling).")
    print(device_msg)
    ...
    opt = optim.Adam(core.parameters(), lr=cfg.learning_rate)  # optimize core params
    mse = nn.MSELoss()
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        epoch_msg = (
            f"[efficiency_close] Epoch {epoch:03d} | "
    ...
    # Time
    (results_dir / "logs" / f"{ticker}_efficiency_close_total_elapsed_s.txt").write_text(f"{total_elapsed:.2f}\n", encoding="utf-8")

    # Peak memory
    if device.type == "cuda":
        peak_mem_mb = torch.cuda.max_memory_allocated() / (1024 ** 2)  # type: ignore[attr-defined]
        (results_dir / "logs" / f"{ticker}_efficiency_close_peak_cuda_memory_mb.txt").write_text(f"{peak_mem_mb:.2f}\n", encoding="utf-8")
    else:
        mem_info = proc.memory_info()
        peak_mb = getattr(mem_info, "peak_wset", mem_info.rss) / (1024 ** 2)
        (results_dir / "logs" / f"{ticker}_efficiency_close_peak_cpu_memory_mb.txt").write_text(f"{peak_mb:.2f}\n", encoding="utf-8")
    ...
    (results_dir / "logs" / f"{ticker}_efficiency_close_cpu_utilization_pct.txt").write_text(f"{cpu_util_pct:.2f}\n", encoding="utf-8")
    ...
    # Loss curves
    tl_path, vr_path = plot_loss_curve(results_dir, train_losses, val_rmses, ticker=f"{ticker}_close")
    with log_path.open("a", encoding="utf-8") as f:
    ...
    done_msg = (
        f"[efficiency_close] Done. Total elapsed: {total_elapsed:.2f}s\n"
        f"[efficiency_close] Test RMSE={m_test.rmse:.2f} MAE={m_test.mae:.2f} MAPE={m_test.mape:.2f}%"
    )
    print(done_msg)

    # One-time artifact hint (quiet)
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_close][TCN efficiency] Done on device: {device}\n")
        f.write(f"[save_metrics] Final metrics in: {metrics_dir}\n")
        f.write(f"[save_metrics] Files: {ticker}_close_val_metrics.(json|txt), {ticker}_close_test_metrics.(json|txt)\n")
        f.write("=" * 80 + "\n\n")

    print(f"[save_metrics] Final metrics in: {metrics_dir}")
    print(f"[save_metrics] Files: {ticker}_close_val_metrics.(json|txt), {ticker}_close_test_metrics.(json|txt)")
