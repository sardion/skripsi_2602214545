...
# -*- coding: utf-8 -*-
"""
process_id_core_inflation_yoy_to_calendar_ts.py

Build a daily calendar time series for Indonesia Core Inflation YoY with:
- calendar_date
- id_core_inflation_yoy (string, preserves % as in raw CSV)
- days_since_id_core_inflation_yoy (int)

Input:
- data/raw/investing_id_core_inflation_yoy_2020-2025_raw.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/id_core_inflation_yoy_ts_cal.csv
...
- days_since_id_core_inflation_yoy:
...
INPUT_FILENAME: Final[str] = "investing_id_core_inflation_yoy_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "id_core_inflation_yoy_ts_cal.csv"
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build id_core_inflation_yoy as strings (preserve '%').
...
def main() -> None:
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "id_core_inflation_yoy": series_vals,                      # keep % strings
        "days_since_id_core_inflation_yoy": days_since_series,     # int
    })
