...
# -*- coding: utf-8 -*-
"""
process_id_bi_rate_to_calendar_ts.py

Build a daily calendar time series for Indonesia BI Rate with:
- calendar_date
- id_bi_rate (string, preserves % as in raw CSV)
- days_since_id_bi_rate (int)

Input:
- data/raw/investing_id_bi_rate_2020-2025_raw.csv
  Columns: Date, Time, Currency, Impact, Event, Actual, Forecast, Previous

Output:
- data/processed/id_bi_rate_ts_cal.csv
...
- Date parsing supports: %m/%d/%Y, %d/%m/%Y, and %Y-%m-%d
"""
...
INPUT_FILENAME: Final[str] = "investing_id_bi_rate_2020-2025_raw.csv"
OUTPUT_FILENAME: Final[str] = "id_bi_rate_ts_cal.csv"
...
def _parse_dates_multi_format(date_series: pd.Series) -> pd.Series:
    ...
    if na_mask.any():
        # As a last resort, try general parser but raise on failure to keep determinism.
...
def build_value_series(calendar: List[date], releases: List[ReleaseRow]) -> List[str]:
    """
    Build id_bi_rate as strings (preserve '%').
...
def main() -> None:
    ...
    out_df: pd.DataFrame = pd.DataFrame({
        ...
        "id_bi_rate": series_vals,                      # keep % strings
        "days_since_id_bi_rate": days_since_series,     # int
    })
