...
# -*- coding: utf-8 -*-
"""
Efficiency scenario trainer (no early stopping, fixed epochs) for LSTM MEAN (AMP) target.
                ...
                for key in mean_keys:
                    if key in df.columns:
                        ...
                        if y_max > y_min:
                            ...
                            return
                # Fallback: compute AMP from OHLC
...
def main() -> None:
    ...
    start_msg = (f"[train_mean][LSTM efficiency] Running on device: {device}")
    print(start_msg)
    ...
    if device.type == "cuda":
        ...
        device_msg = ("[train_mean][LSTM efficiency] CUDA available, running on GPU (AMP enabled, torch profiling).")
    else:
        device_msg = ("[train_mean][LSTM efficiency] CUDA not available, running on CPU (AMP disabled, CPU-only profiling).")
    print(device_msg)
    ...
    # Attach y_min/y_max for denorm-aware metrics (mean/AMP)
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        ...
        # Per-epoch validation in IDR scale
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_mean][LSTM efficiency] Done on device: {device}\n")
