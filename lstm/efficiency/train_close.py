...
# -*- coding: utf-8 -*-
"""
Efficiency scenario trainer (no early stopping, fixed epochs) for LSTM CLOSE target.
...
from metrics import evaluate, save_metrics  # reuse shared (denorm-aware)
...
def main() -> None:
    ...
    start_msg = (f"[train_close][LSTM efficiency] Running on device: {device}")
    print(start_msg)
    ...
    if device.type == "cuda":
        ...
        device_msg = ("[train_close][LSTM efficiency] CUDA available, running on GPU (AMP enabled, torch profiling).")
    else:
        device_msg = ("[train_close][LSTM efficiency] CUDA not available, running on CPU (AMP disabled, CPU-only profiling).")
    print(device_msg)
    ...
    model = LSTMRegressor(input_dim, cfg.hidden_dim, cfg.num_layers, cfg.dropout).to(device)
    ...
    with log_path.open("a", encoding="utf-8") as f:
        ...
        f.write(done_msg + "\n")
        f.write(f"[train_close][LSTM efficiency] Done on device: {device}\n")
