...
# -*- coding: utf-8 -*-
"""
Validation script for the LSTM (stopping_no_embargo): CLOSE-price target.

Conventions aligned with train_close.py:
- Deterministic DataLoader shuffling for train split (though not used here)
- y_min/y_max attach for close price (from metadata or RAW fallback)
...
def make_dataloaders(
    ...
    mirroring train_close.py (deterministic shuffle for train).
    """
...
def main() -> None:
    ...
    start_msg = f"[LSTM stopping_no_embargo][validate_close] Running on device: {device}"
    print(start_msg)
